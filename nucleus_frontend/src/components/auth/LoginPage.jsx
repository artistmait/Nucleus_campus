...
const LoginPage = () => {
  ...
  if (location.state) {
    setFormData({
      moodle_id: location.state.moodle_id || "",
  ...
  if (savedCreds) {
    const { moodle_id, password } = JSON.parse(savedCreds);
    setFormData({ moodle_id, password });
...
const handleSubmit = async (e) => {
  ...
  if (!formData.moodle_id || !formData.password) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-white to-indigo-50">
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-100 px-12 py-14">
        ...
        <div className="mt-10 text-center text-sm text-gray-600">
          ...
          <Link to="/" className="text-indigo-900 font-semibold hover:underline">
            Register here
          </Link>
        </div>
      </div>
